...
int handleCmd(pConnectInfo_t cIn,char *cmd,ppthread_pool_info_t pIn){
    ...
    return 0;
        ...
        int downloadSize;
        recvCycle(fd,&downloadSize,sizeof(int));
         pFdNode_t pNew=(pFdNode_t)calloc(1,sizeof(fdNode_t));
         pNew->downloadSize=downloadSize;
         pNew->newFd=fd;
         pNew->nowdir=cIn->nowDir;
...
restart:
        ...
        sprintf(query,"%s%s%s%s%s%s%s%s%s%s%s%s%s%s%ld%s",query,"'",cIn->userName,"'",",","'",salt,"'",",","'",cryp,"','",tocken,"',",now,")");
        #ifdef DEBUG
        printf("%s\n",query);
        #endif
        ...
        if(res){
            ...
            if(NULL==row){
                ...
                sprintf(query,"%s%ld%s%s%s",query,now, " where tocken='",tocken,"'");
                #ifdef DEBUG
                printf("%s\n",query);
                #endif
                int t=mysql_query(conn,query);
